...
bool art::next_pair_read_indel_cmp(seqRead& read_1, seqRead& read_2){
    ...
    // HD: changed this line in ext_pair_read_indel_cmp_cvg.
    long pos_1=(long) floor((ref_seq.length()-fragment_len)*r_prob());
    //long pos_2=pos_1+fragment_len-read_len;
...
//paired-end read: the second read is reverse complemenaty strand
// HD: this function introduces a coverage bias according to given distribution
bool art::next_pair_read_indel_cmp_cvg(seqRead& read_1, seqRead& read_2, cvgdist &dist){
    if(read_len>ref_seq.length()){
	    return false; //ref_seq is too short.
    }
    int fragment_len=1;
    ...
    // HD: sample read position using given coverage pattern (rather than uniformly).
    long pos_1 = dist.get_ran_pos_weighted();
    while (pos_1 > ref_seq.length()-fragment_len) {
      pos_1 = dist.get_ran_pos_weighted();
    }
    // HD: legacy code without coverage weighting:
    //long pos_1=(long) floor((ref_seq.length()-fragment_len)*r_prob());
    // HD: choose pos_2 from the same strand as read_1, not the opposite
    long pos_2 = pos_1 + fragment_len - read_len;
    // HD: legacy code that chose pos_2 from the opposite strand:
    //long pos_2 = ref_seq.length()-pos_1-fragment_len;
    int slen_1 = read_1.get_indel(read_len);
    int slen_2 = read_2.get_indel(read_len);
    ...
    if(is_plus_strand){
        ...
        read_2.is_plus_strand=false;
//      pos_2=ref_seq.length()-pos_2-read_len;
        read_2.seq_ref = ref_seq_cmp.substr(ref_seq.length()-(pos_2+read_len-slen_2), read_len-slen_2);
        // HD: legacy code that chose pos_2 from the opposite strand:
        //read_2.seq_ref=ref_seq_cmp.substr(pos_2, read_len-slen_2);
    }
    else{
        read_1.is_plus_strand=false;
        read_1.seq_ref=ref_seq_cmp.substr(ref_seq.length()-(pos_1+read_len-slen_1), read_len-slen_1);
//      pos_2=ref_seq.length()-pos_2-read_len;
        read_2.is_plus_strand=true;
        read_2.seq_ref=ref_seq.substr(pos_2, read_len-slen_2);
    }
