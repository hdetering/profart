...
//    bool use_org_qscore = false;
    bool use_cvg_profile = false; // HD: whether to apply uneven read sampling
    ...
    // HD: param for custom coverage pattern
    char* cvg_file = (char*) "";
...
//		i++;
  ...
  // HD: set coverage profile file name
  if (!strcmp(arg, "--cvgProf") || !strcmp(arg, "-cp")) {
    i++;
    cvg_file = argv[i];
    use_cvg_profile = true;
  } else {
		arg_success = false;
...
//	cout << "  -qo  --qOrig    indicate to output the original quality scores regardless the original scores were shifted or not" << endl;
  ...
  cout << "  -cp  --cvgProf  file containing the coverage profile (for uneven read sampling)." << endl;
...
//	cout << "  Example: art --paired --in reference_DNA.fa --out sim2 --len 35 --fcov 2 --mflen 200 --sdev 3.5 -sam" << endl << endl;
    ...
    // HD: import coverage profile to use for weighted read sampling
    cvgdist cdist;
    if (use_cvg_profile) {
      cdist.setdist(cvg_file);
    }
...
//        a_read_2.set_rate(read_len,delRate2,2,a_read_2.del_rate);
        ...
        while(seq_reader.next_seq(id,a_art.ref_seq)){
          // HD: set coverage distributed to current sequence
          if (use_cvg_profile) {
            if (cdist.has_seq(id)) {
              cdist.setseq(id);
              use_cvg_profile = true;
            } else { // coverage profile does not contain current sequence.
              use_cvg_profile = false;
            }
...
//              osID<<num_seq<<fixed<<setfill('0')<< setw(10)<< t_num_read;
      ...
      else { // HD: use coverage profile if provided
        if (use_cvg_profile)
          has_reads=a_art.next_pair_read_indel_cmp_cvg(a_read, a_read_2, cdist);
			  else
          has_reads=a_art.next_pair_read_indel_cmp(a_read, a_read_2);
      }
		}
		if (!has_reads){
			...
			sR.flag=0x01 | 0x02 | 0x40;
		       	...
		       	if(a_read.is_plus_strand){
				       	...
				       	sR2.flag =sR2.flag |0x10;
								// HD: we assigned read 2 pos in art::next_pair_read_indel_cmp_cvg
